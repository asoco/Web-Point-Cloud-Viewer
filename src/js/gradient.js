...
function getGradient(gradientType) {
    let colors = []
    let bounds = [];
    gradientType.forEach(el => {
        colors.push(el[1]);
        bounds.push(el[0]);
    });
    for (let i = colors.length - 1; i < 17; i++) {
...
function getGradientNames() {
    return Object.keys(Gradients);
}

export {Gradients, getGradient, getGradientNames};
