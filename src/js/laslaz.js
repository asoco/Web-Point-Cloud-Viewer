...
(function(scope) {
	...
	window.startNaCl = function(name, tc, config, width, height) {
		...
		if(!common.browserSupportsNaCl()) {
			let event = new CustomEvent('nacl.error', {detail:{message: 'NaCl support is not available'}});
		...
		function(e) {
			console.log("Failed!");
			let event = new CustomEvent('nacl.error', {detail:{message: 'Could not allocate persistant storage'}});
			document.dispatchEvent(event);
		});

		document.addEventListener("nacl.available", function() {
